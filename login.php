...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  $select = "SELECT * from cashad_hub_users where username = '$username' and password = '$password' ";
  $results = $conn->query($select)or
  ...
  if($results->num_rows > 0){
    ...
    if($rs["status"] == "Banned"){
      ...
      $_SESSION["text"] = "Your account has been banned";
      header("location: user-assets/lock");
    }else if($rs["status"] == "Suspended"){
      session_start();
      $_SESSION["username"] = $username;
      $_SESSION["password"] = $password;
      $_SESSION["text"] = "Your account has been Suspended";
      header("location: user-assets/lock");
    }else if($rs["acstatus"] == "Not Activated"){
      $_SESSION["username"] = $username;
      $_SESSION["password"] = $password;
      $_SESSION["text"] = "Your account has not been Activated";
       header("location: user-assets/lock");
    }else{
      $merge = "SELECT * from cashad_hub_merge where userid = '$userid' ";
      $mergeRes = $conn->query($merge)or
      die(myslqi_error($conn));
      if($mergeRes->num_rows > 0){
      $mergeRs = $mergeRes->fetch_assoc();
      $type = $mergeRs["type"];
        if($type == "Payout"){
          $_SESSION["username"] = $username;
          $_SESSION["password"] = $password;
          header("location: user-assets/payment");
        }else if($type == "Receive"){
          $_SESSION["username"] = $username;
          $_SESSION["password"] = $password;
          header("location: user-assets/wallet");
        }
      }else{
        session_start();
        $_SESSION["username"] = $username;
        $_SESSION["password"] = $password;
        header("location: user-assets/dashboard.php");
      }
    }
...
<section class="main-bg">
    <div class="login-container">
        <form method="post" action="<?php echo(htmlspecialchars($_SERVER["PHP_SELF"])); ?>">
            <img src="assets/reset/images/profile-pic.svg" class="avatar" alt="">
            <h2>Welcome</h2>
            <?php flash() ?>
            <div class="input-div one">
                <div class="i"><i class="fa fa-user"></i></div>
                <div>
                    <h5>Username</h5>
                    <input type="text" name="username" id="" class="input" required="required">
                </div>
            </div>
            <div class="input-div two">
                ...
                <div>
                    <h5>Password</h5>
                    <input type="password" name="password" id="" class="input" required="required">
                </div>
            </div>
            <a href="#" class="forget-pass"><i class="fad fa-key"></i>Forget password</a>
        ...
        <div class="media-container-row align-center mbr-white">
            ...
            <div class="row row-copirayt">
                <p class="mbr-text mb-0 mbr-fonts-style mbr-white align-center display-7">
                    © Copyright 2019 - All Rights Reserved by CashFlow
                </p>
            </div>
        </div>
    </div>
